        ...
        while (getline(file, line)) {
            ...
            if (pos == string::npos)
            {
                ...
                for (int i = 0; i < KOR_MAX; i++) {
                    ...
                    if (pos == string::npos) {
                        if (cnt == KOR_MAX - 1) // 한글 뜻 개수 제한 - 1만큼 있는 상태에서 쉼표가 더 이상 없는 경우
                            continue;
                        else
                            break;
                    }
...
void pushWrongQuizWord(QuizWord wrongWord) {
    ...
    // 영어와 한글 뜻이 같은 영단어가 wrongWordList에 존재하는지 검사
    for (it = wrongWordList.begin(); it != wrongWordList.end(); ++it) {
        if (it->eng == wrongWord.eng && it->kor[0] == wrongWord.kor) {
            exist = true;
            break;
        }
    }
    ...
    else { // 영단어가 있어도 뜻을 업데이트 하지 않음
        cout << "해당 단어는 이미 오답노트에 존재합니다." << endl; // 존재하는 단어 처리 여기로 옮김
    }
    exist = false;
...
void pushWrongAccentQuizWord(Word wrongWord) {
    ...
    // 영단어가 wrongAccList에 존재하는지 검사
    for (it = wrongAccList.begin(); it != wrongAccList.end(); ++it) {
        //if (wrongWord.equals(*it)) { // 모든 요소가 같은지 검사할 때
        if(wrongWord.eng == it->eng) {
            exist = true;
            break;
        }
    }
    ...
    if (!exist) { // 영단어가 존재하지 않으면 오답노트에 추가
        wrongAccList.push_back(wrongWord);
    }
    else { // 영단어가 있으면 뜻을 업데이트
        cout << "해당 단어는 이미 오답노트에 존재합니다." << endl; // 존재하는 단어 처리 여기로 옮김
        wrongAccList.erase(it);
        wrongAccList.push_back(wrongWord);
    }
    exist = false;

    // 영단어가 wrongAccHistoryList에 존재하는지 검사
    ...
    if (!exist) { // 영단어가 존재하지 않으면 오답노트에 추가
        wrongAccHistoryList.push_back(wrongWord);
    }
    else { // 영단어가 있으면 뜻을 업데이트
        wrongAccHistoryList.erase(it);
        wrongAccHistoryList.push_back(wrongWord);
    }
}
